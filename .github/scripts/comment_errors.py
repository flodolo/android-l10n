...
class QueryAuthors:
    ...
    def extract_artifact(self, run_id):
        ...
        for a in r.json()["artifacts"]:
            if str(a["workflow_run"]["id"]) == run_id:
                file_url = a["archive_download_url"]
        ...
        if file_url:
            ...
            if not os.path.exists(local_artifact_path):
                print(f"File is missing and can't be downloaded: {local_artifact_path}")

            # Extract the ZIP file
            print(f"Extracting {local_artifact_path}")
    ...
    def exclude_reported_errors(self, new_errors):
        ...
        for comment in comments:
            # Check if the comment was generated by automation, and the
            # associated run ID
            run_id = id_pattern.findall(comment["body"])
            if run_id:
                run_id = run_id[0]
                reported_errors = self.extract_artifact(run_id)

        missing_errors = {}
        for filename, errors in new_errors.items():
            for error in errors:
                if error not in reported_errors.get(filename, {}):
                    if filename in missing_errors:
                        missing_errors[filename].append(error)
                    else:
                        missing_errors[filename] = [error]
...
def main():
    ...
    missing_errors = query_authors.exclude_reported_errors(errors)
